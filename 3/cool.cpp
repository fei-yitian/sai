...
public:
    ...
    vector<pair<int, int>> indices; // 该字符串在 a 数组中的索引
    ...
    void findDuplicates(const vector<vector<string>>& a) {
        ...
        // 查找重复的字符串
        for (auto it = sp_loop.begin(); it != sp_loop.end(); ++it) {
            if (it->second.size() > 1) { // 如果该字符串出现超过一次
                indices = it->second;
            }
        }
        for(size_t j = 0;j < indices.size(); ++j){
            cout << "数组 " << indices[j].first << " 的第 " << indices[j].second << " 个元素; "<<endl;
        }
    }
    ...
    void print_result4(){//我还没有考虑重复三次及以上的情况
        ...
        findDuplicates(can_oscillateCycles_Signals);
        ...
        for(size_t i = 0; i < can_oscillateCycles_Signals.size(); ++i){
            ...
            cout << "  Loop Breaker: ";
            int x=0;
            for(size_t j = 0; j <= i && j < indices.size(); ++j){
                if(i == indices[j].first){
                    //cout<<前一个门结点（不带port的）;
                    cout<<".port0-";
                    cout<<can_oscillateCycles_Signals[i][indices[i].second];
                    cout<<"-Register-";
                    cout<<can_oscillateCycles_Signals[i][indices[i].second];
                    cout<<"'-";
                    //cout<<后一个门结点（带port的）;
