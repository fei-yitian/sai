...
public:
    ...
    void findDuplicates(const vector<vector<string>>& a, vector<vector<pair<int, int>>>& indices) {
        ...
        for(size_t i = 0;i < indices.size(); ++i){
            for(size_t j = 0;j < indices[i].size(); ++j){
                cout << "数组 " << indices[i][j].first << " 的第 " << indices[i][j].second << " 个元素; "<<endl;
            }
        }
    }
    ...
    void print_result4(){//我还没有考虑重复三次及以上的情况
        ...
        for(size_t i = 0; i < can_oscillateCycles_Signals.size(); ++i){
            ...
            for (const auto& duplicate : indices) {
                for (const auto& index : duplicate) {
                    if (index.first == i) {
                        //cout << "重复！";
                        cout<<前一个门结点（不带port的）;
                        cout << ".port0-" << can_oscillateCycles_Signals[i][index.second];
                        cout << "-Register-" << can_oscillateCycles_Signals[i][index.second] << "'-";
                        cout<<后一个门结点（带port的）;
                        cout << endl;
                        found = true;
                    }
            ...
            if (!found) {
                cout<<前一个门结点（不带port的）;
                cout << ".port0-" << can_oscillateCycles_Signals[i][0];
                cout << "-Register-" << can_oscillateCycles_Signals[i][0] << "'-";
                cout<<后一个门结点（带port的）;
                cout << endl;
            }
